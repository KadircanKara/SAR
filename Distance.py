...
def min_cell_visits(sol:PathSolution):
    drone_num_cells = np.append(abs(np.diff(np.array(sol.start_points))), (sol.info.number_of_cells * sol.info.min_visits - sol.start_points[-1]))
...
def max_cell_visits(sol:PathSolution):
    ...
    return max(drone_num_cells) - (sol.info.number_of_cells * sol.info.min_visits) // sol.info.number_of_drones
